...
- **Расширена стартовая онтология до 124 узлов и 138 синонимов (architecture.md §5 п.26).** Исходные 25 категорий Фазы 1 сохранены без изменений (тот же `canonicalKey`), добавлены новые бытовые сферы (сад/дача, стройка, уход, животные, ремонт техники, digital/creative, красота, мероприятия, рукоделие, бизнес-услуги, еда, поручения) плюс синонимы для сопоставления свободных фраз пользователей (`findOntologyNodeForPhrase`). Единственный новый регулируемый узел — `childcare`. Проверено на реальном Postgres (та же схема, что и на этой ветке). **Нужно после деплоя:** повторно выполнить `npm run db:seed` на любой уже засеянной базе — новые узлы не появляются автоматически.

- **Подключены реальные экраны входа/регистрации мобильного клиента и push-регистрация устройства (architecture.md §5 п.27).** Экраны входа/регистрации до этого были Фаза-1 заглушками без единого реального вызова к backend'у — теперь `login`/`register` реально вызывают `POST /auth/login`/`/auth/register`, `select-city.tsx` реально грузит `GET /cities`, токены сохраняются в `SecureStore`, а уже вошедший пользователь при перезапуске приложения теперь действительно попадает в `(tabs)` (раньше — нет, несмотря на восстановление токена). Поверх этого — `ensurePushRegistered()` вызывает `POST /devices` (backend Фазы 5 давно готов, но с мобильного клиента не вызывался ни разу). Отдельно добавлены placeholder-иконки в `apps/mobile/assets/` (папки не существовало вовсе, `eas build` падал бы на этом). **Нужно после деплоя:** ручная проверка на реальном устройстве/Expo Go (вход → push permission prompt → токен появляется в `device_installations`) — в песочнице нет одновременно живого Expo-рантайма и доступного backend'а для e2e прогона.
